...
4. **PYTHONPATH 設定**: `PYTHONPATH=./src` を必須で設定

### **作業ディレクトリ移動コマンド**

```bash
# 必ずdaily_tradeディレクトリに移動してから実行
cd /path/to/daily_trade
pwd  # /path/to/daily_trade であることを確認
```
...
PYTHONPATH=./src uv run python -m daily_trade.scripts.train_model \
  --config config/exp_{id}_model_config.yaml
...
- **LightGBM パラメータのみ**: データセット再利用可能、`model_config.yaml` の `model_params` セクション編集
...
- **PYTHONPATH**: 必ず `PYTHONPATH=./src` を設定してコマンド実行
