...
class RealEstateDataFetcher:
    ...
    def fetch_real_estate(self, year: str, city_code: str) -> dict[str, Any] | None:
        ...
        不動産情報ライブラリが提供する「不動産価格（取引価格・成約価格）情報取得API」から出力される主な項目を整理しました。

        ### 1. 取引の基本情報（キー情報）

        * **種類 (`type`)**: 取引された不動産の種類（中古マンション等、宅地、建物、農地、山林）
        * **取引価格（総額） (`tradePrice`)**: 実際の取引価格（単位：円）
        * **取引時期 (`period`)**: 取引が行われた年と四半期（例：2024年第1四半期）
        * **取引の事情等 (`remarks`)**: 親族間売買や隣地購入などの特殊な事情

        ### 2. 位置・地理情報（メッシュ結合用）

        * **市区町村名 (`municipality`)**: 取引場所の市区町村（例：千代田区）
        * **地区名 (`districtName`)**: 取引場所の地区名（例：飯田橋）
        * **最寄駅：名称 (`nearestStation`)**: 最寄りの鉄道駅の名前
        * **最寄駅：距離（分） (`timeManager`)**: 駅から物件までの徒歩分数

        ### 3. 物件スペック（単価算出・フィルタリング用）

        * **面積（㎡） (`area`)**: 物件の専有面積または土地面積
        * **坪単価 (`unitPrice`)**: 土地取引などの場合に返却される 1 坪あたりの単価
        * **延床面積（㎡） (`totalFloorArea`)**: 建物がある場合の延べ床面積
        * **建築年 (`buildingYear`)**: 建物が完成した年（和暦・西暦の混在に注意）
        * **建物の構造 (`structure`)**: RC（鉄筋コンクリート）、SRC、木造など
        * **用途 (`use`)**: 住宅、事務所、店舗などの利用目的

        ### 4. 都市計画・法的規制（分析レイヤー用）

        * **都市計画 (`cityPlanning`)**: 用途地域（商業地域、第一種低層住居専用地域など）
        * **建ぺい率（％） (`coverageRatio`)**: 敷地面積に対する建築面積の割合
        * **容積率（％） (`floorAreaRatio`)**: 敷地面積に対する延べ床面積の割合
        * **前面道路：方位・種類・幅員 (`direction`, `classification`, `breadth`)**: 物件が接している道路の情報
