...
└─────────────────────────────────────────────────────────────────────┘
  ...
  駅マスタCSV (✅)      数値変換 (✅)         色・高さマッピング (✅)
  区境界GeoJSON (✅)    単価計算 (✅)         HTML出力 (✅)
  ジオコード (✅)       欠損値除去 (✅)       PyVista (⬜)
                      座標生成 (✅)         フレーム出力 (⬜)
...
**実装クラス:** `RealEstateDataFetcher` (`src/real_state_geo_core/data/fetcher.py:13-209`)
...
**実装クラス:** `RealEstateDataFetcher` (`src/real_state_geo_core/data/fetcher.py`)
...
2. **Jupyter Notebook内の追加処理** (`notebooks/01_pydeck_experiment.ipynb`)
...
| モジュール                                   | 要素                          | 行番号          | 概要                                         |
| -------------------------------------------- | ----------------------------- | --------------- | -------------------------------------------- |
| `src/real_state_geo_core/data/fetcher.py`   | `RealEstateDataFetcher`       | 13-209          | データ取得統合クラス                         |
