...
| **4. Visual Core**         | リファレンス画像を入力としたImage-to-Video動画生成         | **Runway Gen-4 Turbo（採用済み: ADR-001）** / Google Veo 3（高品質代替） |
...
- **動画生成プロンプト:** Visual Core用の各シーンの動画生成プロンプト（動作・雰囲気のみ。キャラクター外見はリファレンス画像に委任）
...
**出力:** シーンごとの動画クリップ（24 FPS、Post-Production で最終 FPS に変換）。複数候補の品質ベース選択は T4-1 で実装。
