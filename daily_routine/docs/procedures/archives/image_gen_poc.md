...
プロジェクトルートの `.env.example` をコピーして `.env` ファイルを作成し、APIキーを記入する。
...
```dotenv
DAILY_ROUTINE_API_KEY_STABILITY=your-key
DAILY_ROUTINE_API_KEY_OPENAI=your-key
DAILY_ROUTINE_API_KEY_GOOGLE_AI=your-key
```
...
3 枚の画像セットを一括で Gemini に入力し、以下のスコア（0〜100）を取得する。
...
│       └── back.png
└── evaluation/
    ├── ai_evaluation.json      # Gemini の評価結果
