...
Phase 2: SVG → drawioテンプレート
    │  スキル: /floor_plan_to_video_sub_annotate
    │  ★ ユーザー手作業: drawioで部屋・設備・配置不可を記入
    ▼
Phase 3: drawio → room_info.json + floor_plan_complete.svg
    │  スキル: /floor_plan_to_video_sub_annotate integrate
    ▼
Phase 4: アセット情報の準備
...
```
/floor_plan_to_video_sub_annotate integrate
```
...
**問題があれば**: `/floor_plan_to_video_sub_refine`に戻って配置を修正
...
どのフェーズからでも再開可能。`/floor_plan_to_video_sub_status`で現在の状態を確認できる。
...
A: `assets.json`を編集し、`/floor_plan_to_video_sub_refine`からやり直す。
...
A: `floor_plan_to_video_sub_annotate`スキルの既知の問題。SVG画像のdrawio上での位置ずれが原因。基準点2点のアフィン変換で補正可能。

### Q: レイアウトのセンスが悪い

A: Phase 5で`/floor_plan_to_video_sub_research`を実行してデザインスコア基準を作成する。Phase 6のrefineループで`poc/3dcg_poc3/layout_scorer.py`がGemini Pro 3.0で配置を評価し、「壁面密着」「採光優先度」「中央開放」等の観点でスコアと改善提案を出す。ユーザーのフィードバックを具体的に（「この家具を壁際に」「この通路を広く」等）伝えることでも改善する。
