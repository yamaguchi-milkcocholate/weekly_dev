...
**注意**: C2-ED（環境記述テキスト）は C3-I1 では使用しない。C3 PoC により、環境記述テキストを Flash に渡すとプロンプト肥大化で逆効果となることが確認された（C3-I2 ~136 words vs C3-I1 ~53 words）。C3-T（テキスト環境型 fallback）使用時のみオンデマンドで生成する。
