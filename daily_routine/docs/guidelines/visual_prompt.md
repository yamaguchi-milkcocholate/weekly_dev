...
- [BFL Prompting Guide - Fundamentals](https://docs.bfl.ml/guides/prompting_guide_t2i_fundamentals)
- [BFL Prompting Guide - Essentials](https://docs.bfl.ml/guides/prompting_guide_t2i_essentials)
- [BFL Prompting Guide - Advanced](https://docs.bfl.ml/guides/prompting_guide_t2i_advanced)
- [BFL Prompting Guide - Working Without Negative Prompts](https://docs.bfl.ml/guides/prompting_guide_t2i_negative)
...
プロンプトは以下の4層で段階的に構築する。重要な要素を先に書く（FLUX は先に出現した概念を優先する）。

| 層                               | 役割                           | 優先度 |
| -------------------------------- | ------------------------------ | ------ |
| **Foundation（基盤）**           | 主体 + 動作 + スタイル + 文脈  | 必須   |
| **Visual（視覚）**               | 照明、色彩、構図               | 推奨   |
| **Technical（技術）**            | カメラ設定、レンズ、品質       | 任意   |
| **Atmosphere（雰囲気）**         | 情緒、物語的要素               | 任意   |

**Foundation の構成要素:**

| 要素             | 説明                               | 例                                     |
| ---------------- | ---------------------------------- | -------------------------------------- |
| Subject（主体）  | 誰が・何が映っているか             | `@char`, `A young woman`               |
| Action（動作）   | そのシーンでの状態                 | `sits at a cafe table, holding a latte` |
| Style（スタイル） | 映像的なスタイル                   | `lifestyle photography`                 |
| Context（文脈）  | 場所・環境・時間帯                 | `in @location, first morning light`     |
...
- 1プロンプトにつき1シーンのみ記述する
- **否定表現を使わない**（→ セクション1.3「否定表現の置換」参照）
- **プロンプト長は 30〜80 words が最適**（短すぎると制御不足、長すぎると焦点が散る）
- **「見えている状態」を描写する**（命令や依頼ではなく、映像として見える光景を書く）
- **重要な要素を先頭に置く**（Subject → Action → Style → Context → 付加情報の順）
- 汎用的な動詞（moves, does, goes）を避け、具体的な動詞を使う
- 汎用的な色名（blue, warm colors）を避け、具体的に指定する（cerulean, warm amber）
...
FLUX は否定プロンプトをサポートしない。"no X" と書くと、モデルは X に注目して逆にそれを生成してしまう。

**置換思考の3ステップ:**

1. 除外したい要素を特定する
2. その要素がなかったら代わりに何が見えるかを考える
3. その肯定的な状態を記述する
...
| `no other people`                    | `alone in a quiet room` / `empty, peaceful space`  |
| `no modern elements`                 | `traditional` / `historical setting`               |
| `no text`                            | `clean surfaces` / `unmarked`                      |
| `no cars`                            | `quiet pedestrian walkway with cobblestones`       |
| `without buildings`                  | `pristine wilderness with untouched natural terrain`|
| `not smiling`                        | `stoic expression` / `composed, neutral gaze`      |
...
# OK
A woman with clear unobstructed eyes sits alone in a serene minimalist room,
smooth gradient wall transitioning from soft cream to warm gray.
```
...
# NG: 否定表現
A woman with no glasses sitting in a room with no other people.
→ 修正: A woman with clear unobstructed eyes sits alone in a quiet room, soft natural light.
```
...
## 2. 画像編集プロンプト（Image-to-Image / FLUX Kontext）

### 目的

入力画像をもとに、人物差し替え・スタイル変更・環境調整などの編集を行う。

### T2I との違い

FLUX Kontext は既存画像を文脈として理解し編集するモデル。T2I とは異なり、**指示形式（命令形）のプロンプトが有効**。ただし、否定表現の禁止と具体性の原則は共通。

| 観点           | T2I                          | I2I（Kontext）                       |
| -------------- | ---------------------------- | ------------------------------------ |
| プロンプト形式 | 見えている状態を描写         | 編集指示（何をどう変えるか）         |
| 入力           | なし（テキストのみ）         | 入力画像 + テキスト指示              |
| 否定表現       | 禁止（同じ）                 | 禁止（同じ）                         |
| 具体性         | 必要（同じ）                 | 必要（同じ）                         |
| 保持指示       | 不要                         | **変更しない要素を明示する**         |
| プロンプト長   | 30〜80 words                 | 必要な分だけ（詳しいほど良い）       |

### 基本ルール

- **直接的で具体的な指示を書く**（"Change the background to a beach" のような形式）
- **変更しない要素を明示する**（"keeping the person in the exact same position, scale, and pose"）
- **否定表現を使わない**（T2I と同じ原則。置換思考で肯定表現に変換する）
- **"transform" は使わない**（完全変換のシグナルになる。代わりに "change X to Y" を使う）
- **代名詞を避け、具体的な記述を使う**（"her" → "the woman with short black hair"）
- **1ステップで複雑な変更を詰め込みすぎない**

### プロンプト構造

```
[何を変えるか] + [どう変えるか] + [保持する要素の明示]
```

**例:**

```
Change the background to a sunlit beach while keeping the person
in the exact same position, scale, and pose.
```

```
Change her outfit to a camel-colored long coat with white fur stole
and striped knit top. Maintain the same facial features, hairstyle,
and expression.
```

### キャラクター一貫性のフレームワーク

1. **対象を明確に特定する**: "The woman with short black hair" のように具体的に
2. **変更内容を指定する**: 環境、動作、スタイルなど
3. **同一性マーカーを保持指示に含める**: "maintaining the same facial features, hairstyle, and expression"

### 連鎖編集（Iterative Editing）

Kontext は段階的な編集が得意。ただし以下のルールに従う:

- **1ステップ1変更に絞る**（人物差し替えと照明変更を同時にしない）
- **各ステップで保持する要素を明示する**
- **2〜3ステップ以内に収める**（パスを重ねるほど劣化するリスクがある）

```
# Step 1: 人物差し替え
Change the person to a young Japanese woman with straight dark brown hair
in a low ponytail, wearing a camel coat with white fur stole.
Keep the exact same background, composition, and camera angle.

# Step 2: 照明微調整（前ステップ出力を入力）
Subtly warm the lighting tone to soft golden hour.
Keep the person, pose, background, and composition completely unchanged.
```

### テキスト編集

画像内のテキストを編集する場合、引用符で囲む:

```
Replace "OPEN" with "CLOSED" in the same font style and position.
```

### 避けるべきパターン

```
# NG: "transform" の使用（完全変換として解釈される）
Transform the woman into a viking warrior.
→ 修正: Change her outfit to viking armor with a fur cape. Maintain the same facial features
  and hairstyle.
```
...
# NG: 曖昧な指示
Put him on a beach.
→ 修正: Change the background to a sunlit tropical beach while keeping the person
  in the exact same position, scale, and pose.
```

```
# NG: 否定表現
Do not change the background. No modifications to the face.
→ 修正: Keep the exact same background, facial features, and expression unchanged.
```

```
# NG: 入力画像の外見を過度に再記述
A young Japanese woman, late 20s, slender build, straight dark brown hair,
almond-shaped eyes, fair skin, wearing a camel coat...（入力画像に既にある情報）
→ 修正: The woman in the image walks confidently forward, slight breeze moving her hair.
```

---

## 3. 動画生成プロンプト（Image-to-Video）
...
| `Rembrandt lighting`               | 片側からの45°指向性ライト                                |
| `split lighting`                   | 90°横からの光、ハイコントラスト                          |
...
| `85mm lens`              | ポートレート向け、背景の圧縮効果             |
| `24mm wide angle`        | 広角、空間の広がり                           |
| `35mm film`              | 35mmフィルムの質感                           |
| `anamorphic lens`        | アナモルフィックレンズの歪みとボケ           |
| `f/1.4` / `f/2.8`       | 浅い被写界深度（数値が小さいほどボケが強い） |
| `f/8`                    | 深い被写界深度（全体にピント）               |
...
```
# 人物差し替え（入力画像の構図を保持）
Change the person to a young woman with straight dark brown hair in a low ponytail,
wearing a camel coat with white fur stole over a striped knit top.
Keep the exact same background, composition, camera angle, and lighting.
```

```
# 連鎖編集 Step 1: 人物差し替え
Change the person to a young woman with clear unobstructed eyes,
straight dark brown hair in a low ponytail, composed neutral gaze.
Maintain the exact same background, composition, and camera angle.
```

```
# 連鎖編集 Step 2: 照明微調整（前ステップ出力を入力として使用）
...
- [ ] 重要な要素が先頭に配置されているか（Subject → Action → Style → Context）
- [ ] 場所・環境が明記されているか
- [ ] 1シーンのみを記述しているか
- [ ] 照明・雰囲気が含まれているか
- [ ] 否定表現を使っていないか（肯定的な置換になっているか）
- [ ] 30〜80 words の範囲か
- [ ] 参照画像使用時、参照画像に含まれる情報を過度に再記述していないか
- [ ] 汎用的な動詞・色名を避け、具体的な表現になっているか

### 画像編集プロンプト（I2I / Kontext）

- [ ] 英語で記述されているか
- [ ] 何を変えるかが明確に指示されているか
- [ ] 変更しない要素が明示されているか（保持指示）
- [ ] 否定表現を使っていないか（肯定的な置換になっているか）
- [ ] "transform" を使っていないか（"change X to Y" を使っているか）
- [ ] 代名詞ではなく具体的な記述を使っているか
- [ ] 1ステップに詰め込みすぎていないか
- [ ] 入力画像に既にある情報を過度に再記述していないか
- [ ] 連鎖編集の場合、各ステップの保持指示が含まれているか
