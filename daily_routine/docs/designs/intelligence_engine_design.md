...
[入力] ユーザーが提供するシード動画情報:
  ...
  └── テキスト説明（なぜ重要か、どう活用してほしいか）
    │
    ├──[Step 1] YouTube Data API でメタデータ取得
    │     → タイトル、説明文、タグ、再生数、いいね数
    │
    └──[Step 2] youtube-transcript-api で字幕取得
          → タイムスタンプ付きテキスト
          （字幕なし → yt-dlp で音声DL → Whisper で文字起こし）
    │
    ▼
[シード動画: ユーザー提供情報 + 自動取得情報の統合完了]
...
╚═══════════════════════════════════════════════════════════════╝
    │
    ▼
[Step 3] シード動画のメタデータ + ユーザーテキストをもとに
         YouTube Data API で類似動画を拡張検索
    │
    ▼
[Step 4] 拡張検索動画のメタデータ + 字幕を自動取得
...
╚═══════════════════════════════════════════════════════════════╝
    │
    ▼
[Step 5] 全データをLLMに入力し統合分析
  ├── シード動画: ユーザー画像 + テキスト説明 + 字幕 + メタデータ
  └── 拡張動画: 字幕 + メタデータ
    │
    ▼
[出力] intelligence/report.json（TrendReport）
```
