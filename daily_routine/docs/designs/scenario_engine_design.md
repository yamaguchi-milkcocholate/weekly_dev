...
- **このサブタスクで実現すること:**
  ...
  - Asset Generator 向けの画像生成プロンプトと Visual Core 向けの動画生成プロンプトを自動構築する
...
| プロンプト生成   | LLM にシーン文脈ごと生成させる   | Asset Generator / Visual Core 向けプロンプトは、シーンの状況・カメラワーク・キャラクター情報を統合した文脈依存が必要なため |
...
4. **シーン仕様（`SceneSpec`）:**
   ...
   - `video_prompt`: Visual Core 向け動画生成プロンプト（英語）。キャラクターの**動作・カメラワーク・背景・雰囲気のみ**を記述し、キャラクターの外見描写は含めない（外見はリファレンス画像に委任する）

5. **BGM方向性（`bgm_direction`）:**
   - トレンド分析の `audio_trend`（BPM帯、ジャンル）を反映した自然言語の指示

6. **プロンプト言語:**
   - `image_prompt`, `video_prompt`, `reference_prompt`, `appearance`, `outfit`: 英語（画像/動画生成 AI は英語プロンプトで最高性能を発揮）
...
| Visual Core     | `SceneSpec.video_prompt`         | 動画クリップの生成プロンプト（動作・雰囲気のみ、外見描写なし） |
  ...
  "scenes": [
    {
      ...
      "video_prompt": "A hand reaches out from under a blanket to grab a smartphone on the nightstand, accidentally knocks it off, then quickly sits up in a panic, close-up camera panning up, warm morning light, comedic timing, bright and airy atmosphere"
    },
    {
      "scene_number": 2,
      "duration_sec": 5.0,
      ...
      "video_prompt": "POV shot through a bathroom mirror, hands applying eyeliner in a hurry, making a mistake and wiping it off with a frustrated but comedic expression, bright bathroom lighting, fast-paced morning energy"
    },
    {
      "scene_number": 3,
      "duration_sec": 4.0,
      ...
      "video_prompt": "A person grabs a coffee mug from the counter, takes one sip, checks the time on phone with a shocked face, then rushes toward the entrance with the mug still in hand, follow camera tracking movement, comedic fast pace, bright warm lighting"
    },
    {
      "scene_number": 4,
      "duration_sec": 5.0,
      ...
      "video_prompt": "Wide shot of a figure walking briskly on a train platform, then cut to inside a crowded train holding a strap handle, checking smartphone with one hand, slight swaying motion with the train, natural morning commute atmosphere"
    },
    {
      "scene_number": 5,
      "duration_sec": 6.0,
      ...
      "video_prompt": "A person sits at an office desk, opens a laptop, eyes widen seeing the screen with a comedic shocked reaction, then takes a big sip of coffee, sighs, and starts typing rapidly, close-up on hands and screen, bright office environment"
    },
    {
      "scene_number": 6,
      "duration_sec": 5.0,
      ...
      "video_prompt": "Wide shot of a person sitting at a cafe table with a colleague, eating pasta, laughing and chatting happily, warm cozy lighting, relaxed pace contrasting with the hectic morning scenes, soft natural atmosphere"
    },
    {
      "scene_number": 7,
      "duration_sec": 4.0,
      ...
      "video_prompt": "A person walks out of an office building entrance into a sunset-lit street, stretching arms with a relieved smile, walking with a slight spring in the step while swinging a tote bag, follow camera from behind, warm golden hour lighting, peaceful atmosphere"
    },
    {
      "scene_number": 8,
      "duration_sec": 5.0,
      ...
      "video_prompt": "A person in pajamas sinks into a sofa, hugging a cat-shaped cushion, scrolling on a smartphone, then looks up and smiles warmly at the camera, close-up shot, cozy warm lamp lighting, peaceful ending mood"
    }
  ],
...
| `video_prompt` | `"comedic timing"`, `"comedic shocked reaction"` 等の雰囲気指定 |
