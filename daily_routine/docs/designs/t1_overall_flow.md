...
2. **編集**: シーンごとに使用する衣装バリアント（`variant_id`）を指定
...
**マッピング粒度:** keyframe_mapping はシーン単位で定義する。カット単位のキーフレーム生成時は、同一シーン内の全カットに同じマッピング（character, environment, reference_image, reference_text）を適用する。カットごとに異なるのは `CutSpec.pose_instruction` と `CutSpec.motion_prompt`（Storyboard 由来）のみ。
...
| `schemas/keyframe_mapping.py` | `KeyframeMapping`, `SceneKeyframeSpec`                                                                  | ユーザー入力    | Keyframe                                  |
