...
flowchart TD
    ...
    subgraph "5. Keyframe Engine"
        KE["キーフレーム画像生成<br/>・Gen-4 Image Turbo<br/>・カット単位で生成（1枚）<br/>・キャラクター参照 + スタイル参照（任意） + keyframe_prompt"]
    end
    ...
    SB -.->|"Storyboard"| KE
    SE -.->|"Scenario"| KE
    SM["style_mapping.yaml<br/>（ユーザー手動作成、任意）"] -.->|"StyleMapping"| KE
...
処理は2つの独立したパイプラインステップで実行される。まず KEYFRAME ステップで `front_view` + `CutSpec.keyframe_prompt`（+ 任意のスタイル参照画像）からカット単位でキーフレーム画像を生成し、チェックポイントでユーザー承認を得た後、VISUAL ステップでキーフレーム画像 + `CutSpec.motion_prompt` から動画を生成する。

| データ                      | 使用レイヤー    | 用途                                                                |
| --------------------------- | --------------- | ------------------------------------------------------------------- |
| `CharacterAsset.front_view` | Keyframe Engine | キーフレーム画像生成の参照画像（`@char` タグで参照）                |
| `StyleMapping`              | Keyframe Engine | シーンごとのスタイル参照画像マッピング（`@location` タグ、任意）    |
...
**スタイル参照画像:** ユーザーが `storyboard/style_mapping.yaml` でシーン番号と参照画像を手動で紐付ける。マッピングが存在しないシーンはキャラクター参照のみで生成する（後方互換）。詳細は `style_reference_design.md` を参照。
...
flowchart LR
    ...
    subgraph "Keyframe"
        K_IN["KeyframeInput<br/>(Scenario + Storyboard<br/>+ AssetSet + StyleMapping?)"]
...
| `KeyframeInput`       | Keyframe        | Scenario + Storyboard + AssetSet + StyleMapping?     |
...
| `schemas/style_mapping.py`| `StyleMapping`, `SceneStyleReference`                                                                       | ユーザー入力    | Keyframe                                  |
...
│   └── style_mapping.yaml          # スタイル参照画像マッピング（ユーザー手動作成、任意）
├── assets/
│   ├── reference/                  # ユーザー指定のスタイル参照画像（追加分の配置先）
...
| `style_reference_design.md`       | —            | 5. Keyframe Engine（スタイル参照画像によるキーフレーム品質改善） |
