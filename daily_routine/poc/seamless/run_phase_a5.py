"""Phase A-5: テキスト/オーバーレイ除去（B6）— Python SDK 版実験スクリプト.

seed 画像のテキストオーバーレイ（時刻表示・テロップ）を除去するプロンプトを比較検証する。
入力は seed 画像 1 枚のみ（参照画像なし）。

4パターン × 3seed = 12生成を実行する。

Usage:
    uv run python poc/seamless/run_phase_a5.py --dry-run
    uv run python poc/seamless/run_phase_a5.py --patterns T1,T3
    uv run python poc/seamless/run_phase_a5.py --seeds 1.png,4.png
    uv run python poc/seamless/run_phase_a5.py
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Phase A-5: テキスト/オーバーレイ除去検証")
    parser.add_argument(
        "--patterns",
        type=str,
        default=None,
        help="実行パターンをカンマ区切りで指定 (例: T1,T3)。指定なしで全パターン",
    )
    parser.add_argument(
        "--seeds",
        type=str,
...
def print_dry_run(patterns: list[A5Pattern], seed_images: list[str]) -> None:
    """ドライラン: プロンプト・コスト見積もりを表示する."""
    logger.info("=" * 80)
    logger.info("Phase A-5: テキスト/オーバーレイ除去 — ドライラン")
    logger.info("=" * 80)
    ...
    logger.info("入力構成: seed画像 1枚 + テキスト（参照画像なし）")
    logger.info("")
...
def main() -> None:
    ...
    logger.info("=" * 80)
    logger.info(
        "Phase A-5 実験開始: %d パターン × %d seed = %d 生成, 推定コスト: $%.2f",
        len(patterns),
        len(seed_images),
        total,
        total_cost,
    ...
    log_data = {
        "experiment": "phase_a5_text_removal",
        "model": GEMINI_MODEL,
        "aspect_ratio": ASPECT_RATIO,
        "sdk": "google-genai",
        "timestamp": datetime.now().isoformat(),
        "seed_images": seed_images,
        "patterns": [{"id": p.id, "name": p.name, "description": p.description} for p in patterns],
        "results": all_results,
    }
