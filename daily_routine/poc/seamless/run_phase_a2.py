"""Phase A-2: ポーズ変更（Pose Change）プロンプト最適化 — 実験実行スクリプト.

Gemini 3 Pro でポーズ変更の7パターン × 3seed画像 = 21生成を実行する。

Usage:
    uv run python poc/seamless/run_phase_a2.py --dry-run
    uv run python poc/seamless/run_phase_a2.py --patterns P1,P2
    uv run python poc/seamless/run_phase_a2.py --seeds 1.png,4.png
    uv run python poc/seamless/run_phase_a2.py
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Phase A-2: ポーズ変更プロンプト最適化")
    parser.add_argument(
        "--patterns",
        type=str,
        default=None,
...
def print_dry_run(patterns: list[A2Pattern], seed_images: list[str]) -> None:
    """ドライラン: プロンプト・コスト見積もりを表示する."""
    logger.info("=" * 80)
    logger.info("Phase A-2: ポーズ変更プロンプト最適化 — ドライラン")
    logger.info("=" * 80)
...
def main() -> None:
    ...
    logger.info("=" * 80)
    logger.info(
        "Phase A-2 実験開始: %d パターン × %d seed = %d 生成, 推定コスト: $%.2f",
        len(patterns),
        len(seed_images),
        total,
        total_cost,
    ...
    log_data = {
        "experiment": "phase_a2_pose_change",
        "model": GEMINI_MODEL,
