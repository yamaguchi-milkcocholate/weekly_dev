...
- `.blend`ファイル（例: `poc/3dcg_poc1/madori.blend`）— 間取りの3Dモデル
- `layout_proposal.json` — 確定した配置座標（layout-refineスキルの出力）
- GLBファイル群（例: `poc/3dcg_poc2/output/*.glb`）— 家具の3Dモデル
...
レンダリング画像を確認し、問題があればlayout-refineスキルに戻ってplacement_plan.jsonを修正する。
