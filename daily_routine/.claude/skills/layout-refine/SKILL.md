...
- `placement_engine.py` — 配置エンジン（`poc/3dcg_poc3/placement_engine.py`）
...
```bash
uv run python poc/3dcg_poc3/prepare_candidates.py <出力ディレクトリ> --candidates a b c
```
...
3. 配置エンジンを実行:
   uv run python poc/3dcg_poc3/placement_engine.py {candidate_dir}
...
2. `uv run python poc/3dcg_poc3/placement_engine.py <candidate_dir>` 実行
...
3. （任意）`uv run python poc/3dcg_poc3/layout_scorer.py <candidate_dir>` を各候補に実行
...
4. 候補をアーカイブ:
   ```bash
   uv run python poc/3dcg_poc3/save_iteration.py <出力ディレクトリ> v0_candidates --candidates-dir
   ```
5. 勝者を v1 として記録:
   ```bash
   uv run python poc/3dcg_poc3/save_iteration.py <出力ディレクトリ> v1
   ```
...
```bash
uv run python poc/3dcg_poc3/placement_engine.py <出力ディレクトリ>
```
...
```bash
uv run python poc/3dcg_poc3/save_iteration.py <出力ディレクトリ> <version>
```
...
```bash
uv run python poc/3dcg_poc3/layout_scorer.py <出力ディレクトリ>
```
...
```bash
uv run python poc/3dcg_poc3/save_iteration.py <出力ディレクトリ> <現バージョン>
```
...
```bash
uv run python poc/3dcg_poc3/save_iteration.py <出力ディレクトリ> <前バージョン> \
...
`poc/3dcg_poc3/placement_engine.py` の動作:
