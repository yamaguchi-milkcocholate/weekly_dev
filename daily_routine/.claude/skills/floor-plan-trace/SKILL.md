...
間取り図のPNG画像にはテキスト、家具アイコン、方位記号など多くの要素が含まれる。potraceでベクター化した後、画像を見て不要パスを判断・除去することで、後続の要素分類（`floor-plan-elements`）に適したクリーンなSVGを生成する。
...
`.claude/skills/floor-plan-trace/scripts/svg_trace_tools.py` を使用する。
...
```bash
uv run .claude/skills/floor-plan-trace/scripts/svg_trace_tools.py index "$WORKDIR/${STEM}_trace.svg" -o "$WORKDIR"
```
...
| 太い直線・矩形の構造体 | **保持** | 壁線 |
| 小さい正方形の構造体 | **保持** | 柱 |
| ドアの弧（開閉軌跡） | **保持** | 扇型の弧線（floor-plan-elementsで使用） |
| 部屋を区切る線 | **保持** | 間仕切り壁 |
...
```bash
uv run .claude/skills/floor-plan-trace/scripts/svg_trace_tools.py keep "$WORKDIR/${STEM}_trace_indexed.svg" --ids path_001,path_002,... -o "$WORKDIR"
```

削除対象が少ない場合は `remove` を使用:
```bash
uv run .claude/skills/floor-plan-trace/scripts/svg_trace_tools.py remove "$WORKDIR/${STEM}_trace_indexed.svg" --ids path_045,path_046,... -o "$WORKDIR"
```
...
生成されたクリーンSVG（`{stem}_floor_plan.svg`）は `floor-plan-elements` スキルの入力として使用する。
