...
| サブコマンド | 用途 |
|---|---|
| `template` | PNG背景埋め込みSVGテンプレート生成（walls/pillars/doors/fixturesの4グループ） |
...
出力: `{stem}_template.svg`（PNG背景埋め込み + 空のwalls/pillars/doors/fixturesグループ）
...
```
あなたは建築要素（壁・柱・ドア）の配置レビューエージェントです。
...
- 現在の要素数: 壁{wall_count}個, 柱{pillar_count}個
...
→ 報告: 「要素配置完了。壁: {N}個, 柱: {N}個。{output_path}」
