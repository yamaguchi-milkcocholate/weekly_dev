...
### ルール1: resume は1回ずつ実行し、毎回必ず結果を確認する

`resume` コマンドは **1回実行するごとに必ず停止** し、以下を行う:

1. 生成されたアイテムの結果を提示する（画像を表示、動画の場合はパスを提示）
2. `AskUserQuestion` でユーザーに確認を取る
3. ユーザーの承認を得てから、次の `resume` を実行する

**「続ける」「先に進む」等の指示があっても、このルールは変わらない。** ユーザーが明示的に「全部スキップして」「確認不要で最後まで進めて」と言った場合のみ、連続実行してよい。

### ルール2: 最後のアイテムの resume 前に全体レビューを実施する

`resume` コマンドは、ステップの最後のアイテムを承認すると **自動的に次のステップに遷移し、次のステップの最初のアイテムを実行する。** この動作は止められない。

したがって、最後のアイテムの `resume` を実行する前に:

1. 全アイテムの生成結果を一覧で提示する
2. `AskUserQuestion` で「次のステップに進みますか？」と確認する
3. ユーザーの承認を得てから `resume` を実行する

**これを怠ると、ユーザーが確認する前に次のステップが開始されてしまう。**

### ルール3: resume の実行前にアイテム残数を常に確認する

`resume` を実行する前に、現在のアイテムが **最後の awaiting_review アイテムかどうか** を判定する。最後のアイテムであればルール2を適用する。
