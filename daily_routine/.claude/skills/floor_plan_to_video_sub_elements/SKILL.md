...
クリーンSVG間取り図（`floor_plan_to_video_sub_trace`出力）をPNG化し、ピクセルデータ分析で壁・柱の建築要素を検出して、個別の`<rect>`要素としてSVGに配置する。
...
- `floor_plan_to_video_sub_trace` で生成されたクリーンSVG（`{stem}_floor_plan.svg`）
...
`.claude/skills/floor_plan_to_video_sub_elements/scripts/element_tools.py` を使用する。
...
- **入力**:
  - クリーンSVG間取り図（`{stem}_floor_plan.svg`）— `floor_plan_to_video_sub_trace`の出力
...
```bash
uv run .claude/skills/floor_plan_to_video_sub_elements/scripts/element_tools.py template "$WORKDIR/{stem}_floor_plan_render.png" -o "$WORKDIR"
```
...
```bash
uv run .claude/skills/floor_plan_to_video_sub_elements/scripts/element_tools.py scan "$WORKDIR/{stem}_floor_plan_render.png" -o "$WORKDIR"
```
