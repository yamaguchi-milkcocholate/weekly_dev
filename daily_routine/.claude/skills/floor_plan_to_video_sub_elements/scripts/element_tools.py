...
使い方:
    # PNG背景付きSVGテンプレートを生成
    uv run .claude/skills/floor_plan_to_video_sub_elements/scripts/element_tools.py template input.png -o output_dir

    # PNG画像から壁の特徴点（端点・交差点）を検出
    uv run .claude/skills/floor_plan_to_video_sub_elements/scripts/element_tools.py scan input.png -o output_dir
