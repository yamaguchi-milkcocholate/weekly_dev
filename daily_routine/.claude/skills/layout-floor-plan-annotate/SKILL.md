---
name: layout-floor-plan-annotate
...
- **引数なし**: テンプレート生成モード
- **`integrate`**: 統合モード（drawio → room_info.json + floor_plan_complete.svg）
...
このSVGは後続のlayout-refineスキルでClaude Codeが画像として読んで空間を認識する基盤となる。
