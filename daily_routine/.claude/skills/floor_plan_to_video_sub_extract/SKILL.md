...
カラーPNG間取り画像から壁・柱の`<rect>`要素を持つSVGまでを一気通貫で生成する。内部で `floor_plan_to_video_sub_trace` と `floor_plan_to_video_sub_elements` を順次実行する。
...
│   ├── walls.json                ← Phase 3: 壁座標データ（sub_refine, sub_annotateが使用）
│   └── floor_plan_meta.json      ← Phase 3: SVG座標メタデータ（sub_annotateが使用）
└── work/                         ← 中間成果物
...
| スクリプト | 所属スキル | サブコマンド |
|-----------|-----------|------------|
| `.claude/skills/floor_plan_to_video_sub_trace/scripts/svg_trace_tools.py` | floor_plan_to_video_sub_trace | `index`, `remove`, `keep` |
| `.claude/skills/floor_plan_to_video_sub_elements/scripts/element_tools.py` | floor_plan_to_video_sub_elements | `template`, `scan` |
...
**`floor_plan_to_video_sub_trace` スキルの手順に従って実行する。** ただしパスを以下のように読み替える:

| floor_plan_to_video_sub_trace の変数 | 本スキルでの値 |
|---|---|
...
**`floor_plan_to_video_sub_elements` スキルの手順に従って実行する。** ただしパスを以下のように読み替える:

| floor_plan_to_video_sub_elements の変数 | 本スキルでの値 |
|---|---|
...
- `floor_plan_to_video_sub_annotate` スキルでの部屋定義に接続
- `floor_plan_to_video_sub_status` スキルでの家具配置パイプラインに接続
